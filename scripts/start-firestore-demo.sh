...
echo "Starting BRMS with demo authentication and Cloud Firestore..."
...
(cd "$ROOT/backend" && "$ROOT/backend/.venv/bin/python" - <<'PY'
...
if not settings.google_application_credentials and not settings.firebase_service_account_json:
    raise SystemExit(
        "Firestore credentials are missing. Set GOOGLE_APPLICATION_CREDENTIALS or "
        "FIREBASE_SERVICE_ACCOUNT_JSON in backend/.env."
    )
get_store().health_check()
print("Firestore credential and connectivity check passed.")
PY
)
...
echo "Firestore-backed API ready: http://127.0.0.1:8000"
